...
static int tscookie_tcpopt_parse(struct tcpopt_context *ctx)
{
	...
	case TCPOPT_WINDOW:
		...
		if (*opsize == TCPOLEN_WINDOW)
			ctx->wscale = *wscale < TCP_MAX_WSCALE ? *wscale :
								 TCP_MAX_WSCALE;
		break;
				      ...
				      bool xdp)
{
	...
	if (hdr->ipv4) {
		...
		if ((hdr->ipv4->frag_off &
		     bpf_htons(IP_DF | IP_MF | IP_OFFSET)) != bpf_htons(IP_DF))
			return XDP_DROP;
	...
	ct = bpf_xdp_ct_lookup(ctx, &tup, tup_size, &ct_lookup_opts,
			       sizeof(ct_lookup_opts));
	if (ct) {
					  ...
					  __u8 wscale)
{
	...
	*buf++ = bpf_htonl((TCPOPT_MSS << 24) | (TCPOLEN_MSS << 16) | mss);
	...
	if (tsopt[0] & bpf_htonl(1 << 4))
		...
		*buf++ = bpf_htonl((TCPOPT_NOP << 24) | (TCPOPT_NOP << 16) |
				   (TCPOPT_TIMESTAMP << 8) | TCPOLEN_TIMESTAMP);
	*buf++ = tsopt[0];
	/* TODO Add refinement for this case, the path should be unreachable
	 * R3 bitwise operator |= on pointer prohibited
	 */
	*buf++ = tsopt[1];

	if ((tsopt[0] & bpf_htonl(0xf)) != bpf_htonl(0xf))
		*buf++ = bpf_htonl((TCPOPT_NOP << 24) | (TCPOPT_WINDOW << 16) |
						...
						void *data_end)
{
	...
	if (hdr->ipv4) {
		...
		value = bpf_csum_diff(0, 0, (void *)hdr->ipv4,
				      hdr->ipv4->ihl * 4, 0);
		if (value < 0)
			return XDP_ABORTED;
	...
	if (tscookie_init((void *)hdr->tcp, hdr->tcp_len, &tsopt_buf[0],
			  &tsopt_buf[1], data, data_end))
		tsopt = tsopt_buf;
	...
	if (hdr->ipv4) {
		if (hdr->ipv4->ihl * 4 > sizeof(*hdr->ipv4)) {
			...
			new_tcp_header =
				data + sizeof(*hdr->eth) + sizeof(*hdr->ipv4);
			__builtin_memmove(new_tcp_header, hdr->tcp,
					  sizeof(*hdr->tcp));
	...
	if (hdr->ipv4) {
		hdr->tcp->check =
			csum_tcpudp_magic(hdr->ipv4->saddr, hdr->ipv4->daddr,
					  hdr->tcp_len, IPPROTO_TCP, value);
		...
		hdr->tcp->check =
			csum_ipv6_magic(&hdr->ipv6->saddr, &hdr->ipv6->daddr,
					hdr->tcp_len, IPPROTO_TCP, value);
	} else {
		return XDP_ABORTED;
	}
...
static __always_inline int syncookie_part1(void *ctx, void *data,
					   void *data_end,
					   struct header_pointers *hdr,
					   bool xdp)
{
...
static __always_inline int syncookie_part2(void *ctx, void *data,
					   void *data_end,
					   struct header_pointers *hdr)
{
	if (hdr->ipv4) {
