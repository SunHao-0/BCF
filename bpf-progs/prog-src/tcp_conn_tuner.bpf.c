...
BPF_MAP_DEF(remote_host_map, BPF_MAP_TYPE_HASH, struct in6_addr,
	    struct remote_host, 1024, 0);

BPF_MAP_DEF(sk_storage_map, BPF_MAP_TYPE_SK_STORAGE, int, __u64, 0,
	    BPF_F_NO_PREALLOC);
...
int conn_tuner_sockops(struct bpf_sock_ops *ops)
{
	...
	struct tcp_conn_event_data *event_data =
		(struct tcp_conn_event_data *)&event.raw_data;
		...
		if (ops->total_retrans > (ops->segs_out >> DROP_SHIFT)) {
			...
			if (!statep || *statep != TCP_STATE_CONG_BBR) {
				...
				bpf_sock_ops_cb_flags_set(
					ops, BPF_SOCK_OPS_STATE_CB_FLAG);
			}
		}
		return 1;
	...
	case BPF_SOCK_OPS_PASSIVE_ESTABLISHED_CB: {
		...
		__u32 cands[NUM_TCP_CONN_METRICS];
		...
		if (ncands > 1 && ncands <= NUM_TCP_CONN_METRICS) {
			...
			/* Refactor from the above
			 * TODO add refinement for this
			 * minindex = cands[choice]; @ tcp_conn_tuner.bpf.c:176
			 * 342: (0f) r1 += r0
			 * math between fp pointer and register with unbounded min value is not allowe
			 * */
			if (choice >= 4)
				return 1;
			minindex = cands[choice];
	...
	case BPF_SOCK_OPS_STATE_CB: {
		...
		__u64 metric, metric_old, min_rtt, rate_interval_us,
			rate_delivered, mss;
		...
		if (!m->min_rtt || min_rtt < m->min_rtt)
			m->min_rtt = min_rtt;
		if (!m->max_rate_delivered ||
		    rate_delivered > m->max_rate_delivered)
		...
		for (i = 0; i < NUM_TCP_CONN_METRICS; i++) {
			if (s == i)
				continue;
			if (remote_host->metrics[i].metric_value <
			    m->metric_value) {
				greedy = false;
				break;
			}
		...
		m->metric_value =
			rl_update(metric_old, metric, BPFTUNE_BITSHIFT);
		m->metric_count++;
		if (greedy)
			m->greedy_count++;
		if (debug) {
			event.tuner_id = tuner_id;
			bpf_ringbuf_output(&ring_buffer_map, &event,
					   sizeof(event), 0);
		}
		return 1;
	}
