...
{
	__u64 stack_depth;
	volatile __s64 last_stack_depth;
	...
	if (entry) {
		...
		if (stack_depth >= FUNC_MAX_STACK_DEPTH - 1)
			return NULL;
		/* The last_stack_depth variable is marked as volatile to increase the difficulty.
		 * We should be able to handle this, once more refinement added.
		 * Currently rejected due to: "math between map_value pointer and register with unbounded min value is not allowed"
		 * The refinement is not even triggered.
		 */
		...
		func_stack->stack_depth = stack_depth + 1;
		func_stack->ips[stack_depth + 1] = 0;
...
static void output_trace(struct pt_regs *ctx, struct trace *trace)
{
	...
	if (trace_len <= sizeof(*trace))
		bpf_perf_event_output(ctx, &ksnoop_perf_map, BPF_F_CURRENT_CPU,
				      trace, trace_len);
skip:
	clear_trace(trace);
}

static void output_stashed_traces(struct pt_regs *ctx, struct trace *currtrace,
				  bool entry)
{
	struct func_stack *func_stack;
	...
	if (entry) {
		 ...
		 */
		for (i = 0; i < func_stack->stack_depth - 1 &&
			    i < FUNC_MAX_STACK_DEPTH;
		     i++) {
	...
	for (i = 0; i < MAX_TRACES; i++) {
		...
		if (currtrace->flags & KSNOOP_F_MEMBER) {
			...
			if (currtrace->flags & KSNOOP_F_PTR) {
				...
				ret = bpf_probe_read_kernel(&data, sizeof(data),
							    dataptr);
				if (ret) {
				...
				bpf_probe_read_kernel(&currdata->raw_value,
						      currtrace->size,
						      (void *)data);
			}
		} else {
