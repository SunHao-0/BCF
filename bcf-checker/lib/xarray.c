...
static inline void *kmem_cache_alloc_lru_wrapper(struct kmem_cache *cachep,
...
bool xas_nomem(struct xa_state *xas, gfp_t gfp)
{
	...
	if (xas->xa->xa_flags & XA_FLAGS_ACCOUNT)
		gfp |= __GFP_ACCOUNT;
	xas->xa_alloc = kmem_cache_alloc_lru_wrapper(radix_tree_node_cachep, xas->xa_lru, gfp);
	if (!xas->xa_alloc)
		return false;
...
static void *xas_alloc(struct xa_state *xas, unsigned int shift)
{
	...
	if (node) {
		...
		node = kmem_cache_alloc_lru_wrapper(radix_tree_node_cachep, xas->xa_lru, gfp);
		if (!node) {
...
void xas_try_split(struct xa_state *xas, void *entry, unsigned int order)
{
	...
	if (xas->xa_shift < node->shift) {
		...
		if (!child) {
			child = kmem_cache_alloc_lru_wrapper(radix_tree_node_cachep,
						     xas->xa_lru, gfp);
			if (!child) {
				xas_destroy(xas);
				xas_set_err(xas, -ENOMEM);
...
/* Userspace replacement for kmem_cache_alloc_lru */
static inline void *kmem_cache_alloc_lru_wrapper(struct kmem_cache *cachep,
                                                 struct list_lru *lru, gfp_t gfp)
{
	return kmem_cache_alloc_lru(cachep, lru, gfp);
}
